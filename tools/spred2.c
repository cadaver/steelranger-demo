...
int main (int argc, char *argv[])
{
  ...
  if (!win_openwindow("Sprite editor 2", NULL)) return 1;
...
void mainloop(void)
{
  for (;;)
  {
    ...
    if (k == KEY_F1) loadspr();
    if (k == KEY_F2) savespr();
    if (k == KEY_F3) loadoldslicespr();
    if (k == KEY_F4) loadoldspr();
    ...
    if (phase == 2)
    {
      ...
      if (r == 1)
      {
        ...
        for (;;)
        {
          ...
          spritedata[frame*64+63] = tempheader.color & 31;
...
void loadoldslicespr(void)
{
  ...
  for (;;)
  {
    win_getspeed(70);
    gfx_fillscreen(254);
    printtext_center_color("LOAD HESSIAN/MW4 SPRITEFILE:",70,SPR_FONTS,COL_WHITE);
    printtext_center_color(ib1,80,SPR_FONTS,COL_HIGHLIGHT);
    if (phase > 1)
    {
      printtext_center_color("LOAD AT SPRITENUM:",95,SPR_FONTS,COL_WHITE);
    ...
    if (phase == 3)
    {
      ...
      if (r == 1)
      {
        ...
        for (c = 0; c < frames; c++)
        {
          ...
          tempheader.color = spritedata[(frame+c)*64+63] & 31;
          ...
          fwrite8(handle, -tempheader.fliphotspotx/2);
          fwrite8(handle, tempheader.connectspotx/2);
          fwrite8(handle, tempheader.flipconnectspotx/2);
